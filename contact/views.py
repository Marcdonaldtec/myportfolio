...
def contact_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            send_mail(
                'Nouveau message de contact from your portfolio',
                f"Vous avez reçu un nouveau message de {contact.full_name}. \nEmail: {contact.email}. \nSujet: {contact.subject}. \nMessage: {contact.message}",
                settings.EMAIL_HOST_USER,
                [settings.EMAIL_HOST_USER],
                fail_silently=True,
            )
            return redirect('contact:contact_detail', pk=contact.id)
    else:
        form = ContactForm()
